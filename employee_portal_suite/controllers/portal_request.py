...
# ---------------------------------------------------------
    ...
    def portal_dashboard(self, **kw):
        ...
        # ==========================================================
        # 1. My Requests (Employee Request count for logged-in user)
        # ==========================================================
        ...
        # Manager / HR / Finance / CEO
        # ==========================================================
        employee_pending_domain = [
            ('state', 'in', ['manager', 'hr', 'finance', 'ceo'])
        ]
        all_employee_pending = request.env['employee.request'].sudo().search(employee_pending_domain)

        employee_pending_count = 0

        for rec in all_employee_pending:

            # Manager stage: Only if this employee is the assigned manager
            if rec.state == 'manager':
                if user.has_group('employee_portal_suite.group_employee_portal_manager') \
                    and rec.manager_id == employee:
        ...
        # ==========================================================
        # 3. Material Pending Approvals
        # Purchase Rep / Store Manager / PM / Director / CEO
        # ==========================================================

        # First determine what stage this user is responsible for
        if user.has_group('employee_portal_suite.group_mr_purchase_rep'):
            ...
            stage_domain = [('id', '=', 0)]  # user cannot see any material approvals

        all_material_pending = request.env['material.request'].sudo().search(stage_domain)

        material_pending_count = len(all_material_pending)
